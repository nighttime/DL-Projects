...
def build_model(data_shape):
	...
	model.add(Dense(hidden_layer_1_size, input_shape=data_shape))

	# Add a nonlinearity so we can learn a deeper layer
	# Using PReLU instead of ReLU allows us to learn the optimal ReLU leak
	model.add(PReLU())
